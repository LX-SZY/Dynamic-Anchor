...
class FCOSHead(AnchorFreeHead):
    ...
    Args:
        ...
        center_sample_radius (float): Radius of center saccmpling. Default: 1.5.
    ...
    def __init__(self,
                 ...
                 regress_ranges=((-1, 64), (64, 128), (128, 256), (256, 512),
                                 (512, INF)),
                 center_sampling=False,    # TRICKS
                 center_sample_radius=1.5,
                 norm_on_bbox=False,       # TRICKS
                 centerness_on_reg=False,  # TRICKS
                 loss_cls=dict(
                     type='FocalLoss',
                     use_sigmoid=True,
                     gamma=2.0,
    ...
    def forward_single(self, x, scale, stride):
        ...
        bbox_pred = scale(bbox_pred).float()                 # 每一层输出乘以一个可学习的参数
    ...
    def loss(self,
        ...
        flatten_cls_scores = torch.cat(flatten_cls_scores)         # 将5层預測值concat 162136*80
        ...
        pos_inds = ((flatten_labels >= 0)                           # 正样本索引
        ...
        pos_bbox_preds = flatten_bbox_preds[pos_inds]               # 预测的正样本
        pos_centerness = flatten_centerness[pos_inds]

        if num_pos > 0:
            pos_bbox_targets = flatten_bbox_targets[pos_inds]
            pos_centerness_targets = self.centerness_target(pos_bbox_targets)          # 生成中心度预测目标
            pos_points = flatten_points[pos_inds]
            pos_decoded_bbox_preds = distance2bbox(pos_points, pos_bbox_preds)         # 解码为左上与右下坐标的形式
            pos_decoded_target_preds = distance2bbox(pos_points,
                                                     pos_bbox_targets)
            # centerness weighted iou loss
            loss_bbox = self.loss_bbox(                            # 计算IOUloss
                pos_decoded_bbox_preds,
                pos_decoded_target_preds,
                weight=pos_centerness_targets,
                avg_factor=pos_centerness_targets.sum())
            loss_centerness = self.loss_centerness(pos_centerness,  # 计算 中心度损失
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        num_points = [center.size(0) for center in points]     # 每层特征图的中心点数量

        # get labels and bbox_targets of each image
        labels_list, bbox_targets_list = multi_apply(             # 为每一张图像构造预测目标
        ...
        for i in range(num_levels):   # 5
            concat_lvl_labels.append(                            # concat 每层征征图的targets
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        num_points = points.size(0)          # 20267
        num_gts = gt_labels.size(0)          # 6
