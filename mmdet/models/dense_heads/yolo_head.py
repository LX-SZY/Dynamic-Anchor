...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.featmap_strides = featmap_strides  # [32, 16, 8]
        self.train_cfg = train_cfg          # 来自yolov3配置文件
        self.test_cfg = test_cfg
        if self.train_cfg:                  # 正负样本分配策略
            self.assigner = build_assigner(self.train_cfg.assigner)   # build正负样本分配策略
            if hasattr(self.train_cfg, 'sampler'):
                sampler_cfg = self.train_cfg.sampler
            else:
                sampler_cfg = dict(type='PseudoSampler')
            self.sampler = build_sampler(sampler_cfg, context=self)  # build 采样策略

        self.one_hot_smoother = one_hot_smoother    # 是否使用label_smoothing[设置为非0，使用label—smoothing]

        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        self.act_cfg = act_cfg

        self.bbox_coder = build_bbox_coder(bbox_coder)  # build bbox编码策略
        self.anchor_generator = build_anchor_generator(anchor_generator)  # build 锚框的生成

        self.loss_cls = build_loss(loss_cls)    # build分类损失函数
        self.loss_conf = build_loss(loss_conf)  # build置信度损失函数
        self.loss_xy = build_loss(loss_xy)      # build中心坐标损失函数
        self.loss_wh = build_loss(loss_wh)      # build长宽损失函数
        # usually the numbers of anchors for each level are the same
        # except SSD detectors
        self.num_anchors = self.anchor_generator.num_base_anchors[0]   # 每层特征图的锚框尺寸为3种
        assert len(
            self.anchor_generator.num_base_anchors) == len(featmap_strides)
        self._init_layers()

    @property
    def num_levels(self):  # 3
        return len(self.featmap_strides)

    @property
    def num_attrib(self):  # 85
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):        # self.num_levels=3
            conv_bridge = ConvModule(           # conv_bridge = “conv”+"norm"+"act"
                self.in_channels[i],
                self.out_channels[i],
                3,
                padding=1,
            ...
            conv_pred = nn.Conv2d(self.out_channels[i],                 # 用1*1的卷积进行预测 ，输出C = 85*3
    ...
    def forward(self, feats):
        ...
        pred_maps = []
        for i in range(self.num_levels):        # feats 高层——低层
            x = feats[i]
            x = self.convs_bridge[i](x)
            pred_map = self.convs_pred[i](x)
            pred_maps.append(pred_map)          # 预测结果

        return tuple(pred_maps),                # 返回预测结果  3层特征图的预测结果
    ...
    def get_bboxes(self,
        ...
        """
        result_list = []
        num_levels = len(pred_maps)  # 3 pred_maps:tuple(B*255*13*19, B*255*26*38,B*255*52*76)
        for img_id in range(len(img_metas)):    # 遍历所有图片进行计算
            pred_maps_list = [                  # 组合三层的预测结果
    ...
    def _get_bboxes_single(self,
        ...
        multi_lvl_anchors = self.anchor_generator.grid_anchors(  # 锚框list[ level0=(H*W*3, 4), level1=(H'*W'*3, 4), level2=(H''*W''*3, 4) ]
        ...
        for i in range(self.num_levels):              # 遍历每一层的预测结果
            # get some key info for current scale
            pred_map = pred_maps_list[i]
            stride = self.featmap_strides[i]

            # (h, w, num_anchors*num_attrib) -> (h*w*num_anchors, num_attrib)
            pred_map = pred_map.permute(1, 2, 0).reshape(-1, self.num_attrib)  # .permute用于tensor的维度交换， 与np.transpose作用一样，只是对象不一样

            pred_map[..., :2] = torch.sigmoid(pred_map[..., :2])   # 映射到0-1
            bbox_pred = self.bbox_coder.decode(multi_lvl_anchors[i],       # (h*w*num_anchors, 4)
                                               pred_map[..., :4], stride)  # 预测结果+anchors映射到真实框
            # conf and cls
            conf_pred = torch.sigmoid(pred_map[..., 4]).view(-1)   # 置信度预测 (h*w*num_anchors, 1)
            cls_pred = torch.sigmoid(pred_map[..., 5:]).view(      # 类别预测 (h*w*num_anchors, 80)
                -1, self.num_classes)  # Cls pred one-hot.

            # Filtering out all predictions with conf < conf_thr   # 过滤掉低于置信度阈值的预测结果
            ...
            if conf_thr > 0:
                ...
                conf_inds = conf_pred.ge(conf_thr).nonzero(  # 获取置信度大于阈值的index
                    as_tuple=False).squeeze(1)
                bbox_pred = bbox_pred[conf_inds, :]
                cls_pred = cls_pred[conf_inds, :]
                conf_pred = conf_pred[conf_inds]

            # Get top-k prediction                     # 如果预测结果多余1k,取置信度前top k 个预测结果
            ...
            # Save the result of current scale              # 将预测结果添加到list中
            multi_lvl_bboxes.append(bbox_pred)
            multi_lvl_cls_scores.append(cls_pred)
            multi_lvl_conf_scores.append(conf_pred)
            # 将每层特征图的预测数量添加到列表中
            size_stage.append(bbox_pred.shape[0])
            ###########################################
        # Merge the results of different scales together    # 组合三层的预测结果
        ...
        # i.e., the last column.                                              # 添加背景类到类别预测结果中
        ...
        if with_nms and cfg.get('nms', None) is not None:
            det_bboxes, det_labels, ind = multiclass_nms(         # 执行多类别NMS
                multi_lvl_bboxes,
                multi_lvl_cls_scores,
                cfg.score_thr,
                cfg.nms,
            ...
            for k in ind:
                if 0 <= k < size_stage[0]:
                    k = 0
                elif size_stage[0] <= k < size_stage[1]:
                    k = 1
                else:
                    k = 2
                stage.append(k)

            return det_bboxes, det_labels, stage
